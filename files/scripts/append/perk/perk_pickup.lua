...
function perk_pickup(entity_item, entity_who_picked, item_name, do_cosmetic_fx, kill_other_perks,
  ...
  local perk_entity_ids = EntityGetAllChildren(entity_who_picked, "perk_entity")
  ...
  if customized_perk then
    local ui_icon_compoennt_id = EntityGetFirstComponentIncludingDisabled(newer_perk_entity_id,
      "UIIconComponent")
